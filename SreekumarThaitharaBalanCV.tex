...
&\small{
\begin{itemize}
    %\item Raised money for a time sereis forecasting platform based on Bayesian Deep Learning by developing a prototype and presenting the results and prospects to investors.
    %\item Hired and managed a high quality machine learning researchers and software engineers.
  %   \item Designed a robust time series prediction testing platform for evaluating the performance of algorithms. Wrote library in Python for generating time series based on stochastinc processes. Designed a preformance analysis suite for benchmarking the time series predictions and their uncertainties.
  %   \item Oversaw the development of a feature genration library in Python. This included developing interfaces for handling columnar data structures and their manipulation, e.g. resampling and applying various integral transformations to raw data using Pandas, xarray, scikit-learn and pymc3.
  %   \item Supervided the systematic development of a Bayes-by-backprop Deep Learning algorithm for time series forecasting. The prototyping was done using sythetic time series library as well as noise-amplified MNIST data. The stack was written completely in TensforFlo with a 3D architechure using convolutional, pooling and fully connected layers with relu and selu activation functions. Implemeted a normalised binning scheme for tranforming the regression to classification problem for better signal to noise.
  %   \item Oversaw the design, development and deployment a robust timesries sequential testing platform for both local and cloud platforms. The library moved through time series data and interfaced the Deep Leraning oracle to perform training and produce predictions based on a presscription parsed from yaml files. For parallelising the hyper parameter optimisation I supervided the development of an Asible playbook for launching and running several tens of GPU-instances in the cloud.
  % \item Researched and implemted a library for estimating covariance matrices from noisy samples. This included taking already availbe C code and interfaing it with python through Cython as well as developing custom modifications to suite the outputs from time series predictions.
  %   \item Supervised the Git workflow, test driven development using CircleCI and eventual release of packages in GemFury.
  %   \item Lieased with customers on weekly calls and moulded the software according to the customer's specifications. This included writing a wrapper process for interfacing with client's data and bitcompiling the relevant parts of Python code using Nuikta library.
